...
class TestVGCDesignConstraints(unittest.TestCase):
    ...
    def test_check_every_rule(self):
        ...
        constraints.add_pkm_rule(roster[2], MoveRosterFixedSizeRule(roster, roster[2].move_roster))
        constraints.add_pkm_rule(roster[3], MovesUnchangeableRule(roster, roster[3].move_roster))
        ...
        roster[3].move_roster.pop()
        failed = constraints.check_every_rule(roster)
        self.assertEqual(len(failed), 3)
        roster[2].move_roster.pop()
        failed = constraints.check_every_rule(roster)
