...
# monkey‐patch BasicMeta so we can peek at the last match result
from vgc2.meta import BasicMeta

_orig_add_match = BasicMeta.add_match
def _hooked_add_match(self, teams, winner, elos):
    # store (teams, winner, elos) for the next decision call
    self._last_match = (teams, winner, elos)
    return _orig_add_match(self, teams, winner, elos)

BasicMeta.add_match = _hooked_add_match
...
def query_llm(prompt):
    ...
    try:
        ...
        if not response.ok or "choices" not in resp_json:
            ...
            return []
        # 1) Obtén el contenido generado
        content = resp_json["choices"][0]["message"]["content"]
        logging.info("LM Studio respondió (crudo): %s", content)

        # 2) Elimina todo lo que esté entre <think>...</think> (modo DOTALL e insensible a mayúsculas)
        content = re.sub(r'(?is)<think>.*?</think>', '', content).strip()
        logging.info("Contenido limpio: %s", content)

        # 3) Extrae los números del contenido limpio
...
class MCTSTeamBuildPolicy(TeamBuildPolicy):
    ...
    def decision(self,
        ...
        logging.info("Enviando prompt a LM Studio:\n%s", prompt)
        indices = query_llm(prompt)
        logging.info("Indices recibidos: %s", indices)

        # ─── Filtrado de índices válidos ───
        valid = [i for i in indices if 0 <= i < len(roster)]
        if len(valid) < max_team_size:
            logging.warning("Algunos índices estaban fuera de rango, aplicando fallback: %s", indices)
            # añadimos índices aleatorios que falten
        ...
        logging.info("Índices válidos tras filtrado: %s", valid)

        # 3) Guardamos para la próxima ronda
        ...
        # 5) Log final
        final_fmt = [fmt_pokemon_species(roster[i]) for i in valid]
